...
func smallestTASNodeCPU(clients ...client.Client) resource.Quantity {
	...
	for _, k8sClient := range clients {
		cpu := getTASWorkerNode(k8sClient).Status.Allocatable[corev1.ResourceCPU]
		if smallest == nil || cpu.Cmp(*smallest) < 0 {
			cpuCopy := cpu.DeepCopy()
			smallest = &cpuCopy
		}
	}
	gomega.Expect(smallest).NotTo(gomega.BeNil())
	return *smallest
}
...
func waitForManagerCentralizedAdmission(wlKey types.NamespacedName) (clusterName string, levels []string) {
	...
	gomega.Eventually(func(g gomega.Gomega) {
		...
		g.Expect(ta.Levels).To(gomega.ContainElement(constants.MultiKueueClusterLabel))
