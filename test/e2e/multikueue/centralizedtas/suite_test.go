...
var _ = ginkgo.BeforeSuite(func() {
	...
	expectCentralizedTASEnabled()
})

func expectCentralizedTASEnabled() {
	ginkgo.GinkgoHelper()
	config := &corev1.ConfigMap{}
	gomega.Eventually(func(g gomega.Gomega) {
		g.Expect(k8sManagerClient.Get(ctx, client.ObjectKey{Namespace: kueueNS, Name: "kueue-manager-config"}, config)).To(gomega.Succeed())
		configYAML, found := config.Data["controller_manager_config.yaml"]
		g.Expect(found).To(gomega.BeTrue())
		g.Expect(configYAML).To(gomega.ContainSubstring("MultiKueueCentralizedTAS: true"),
			"manager must enable the MultiKueueCentralizedTAS feature gate; deploy with E2E_CONFIG_FOLDER=multikueue/centralizedtas")
	}, util.Timeout, util.Interval).Should(gomega.Succeed())
}
